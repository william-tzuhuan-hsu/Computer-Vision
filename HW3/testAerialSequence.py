...
#         plt.plot(motion[1],motion[0], '.', markersize=2)
#         # plt.show()
#         plt.savefig("../result/aerial_{i}.png".format(i=i))
#         plt.close()

start_time = time.time()
for i in range(seq.shape[2]-2):
# for i in range(10):
    mask = SubtractDominantMotion(seq[:,:,i], seq[:,:,i+1], threshold, num_iters, tolerance)
    motion = np.where(mask==True)
    print(motion)
    if (i+1) % 10 == 0:
        ...
        plt.plot(motion[1],motion[0], '.', markersize=2)
        # plt.show()
        plt.savefig("../result/aerial_inv_{i}.png".format(i=i))
        plt.close()
